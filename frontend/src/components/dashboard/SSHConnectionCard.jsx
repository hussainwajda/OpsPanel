...
import { Server, TerminalSquare, User, Lock, Eye, EyeOff, X, Key, FileText, Info } from "lucide-react"
...
const SSHConnectionCard = ({
  ...
  const handleConnect = () => {
    ...
    const connectToServer = async () => {
      ...
      if (authMethod === "password") {
        ...
        if (!encryptedPassword) throw new Error("Failed to encrypt password.")
        ...
        if (!encryptedKeyContent) throw new Error("Failed to encrypt key file.")
        payload.keyFileContent = encryptedKeyContent
      ...
      // Trigger connection history refresh if callback is provided
      if (onConnectionSuccess) {
        console.log('Triggering connection history refresh after successful connection')
        onConnectionSuccess()
      }
  ...
  return (
    <div
      className={`w-full select-none max-w-sm bg-black/20 backdrop-blur-xl p-6 rounded-2xl border ${initialConnected ? "border-green-500/30" : "border-gray-900/50"} shadow-2xl connection-card`}
    >
      <div className="flex justify-between items-center mb-5">
        <div className="flex items-center">
          <TerminalSquare
            className={`h-7 w-7 ${initialConnected ? "text-green-400" : "text-white"} mr-3 terminal-icon`}
          />
          <div>
      ...
      <form onSubmit={(e) => e.preventDefault()} className="space-y-3">
          ...
          <div className={`relative mb-1 ${authMethod === "password" ? "opacity-100" : "opacity-50"}`}>
            <div className="flex items-center mb-2">
              ...
              <label
                ...
                className={`ml-2 text-sm text-white flex items-center ${initialConnected ? "cursor-not-allowed" : "cursor-pointer"}`}
              >
            ...
            {/* Password Input */}
            {(authMethod === "password" ||
              (initialConnected && localStorage.getItem("serverAuthMethod") === "password")) && (
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
        ...
        <div className="flex items-center text-xs text-gray-400 mt-4">
          ...
          <p>
            Your connection details are securely stored in your browser for convenience. No data is sent to external
            servers.
          </p>
        </div>
